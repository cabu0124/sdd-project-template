...
| `conventional-commit` and `spec-pointer` required as status checks | a check that can be skipped is documentation |
...
`conventional-commit` in `.github/workflows/pr-title.yml`, `spec-pointer` in
`.github/workflows/spec-pointer.yml`. Requiring a
