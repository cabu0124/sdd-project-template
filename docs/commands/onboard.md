...
`docs/standards/`; they contain no project decisions of their own.
...
- **Each command wrapper suggests a model and asks, and nothing sets one.** The
  ...
  | --- | --- | --- | --- | --- |
  | `high` | The commands that decide: `specify`, `clarify`, `plan`, `analyze`, `adopt` | `opus` | `Claude Opus 5` | `gpt-5.5` |
  | `standard` | The commands that execute what was decided: `tasks`, `implement`, `init` | `sonnet` | `Claude Sonnet 5` | `gpt-5.4` |
  | `mechanical` | The commands that run a script: `sync`, `preflight`, `onboard` | `haiku` | `Claude Haiku 4.5` | `gpt-5.4-mini` |
...
- Ask for a new endpoint. The agent must write or locate its spec and stop
  before code when no approved spec exists.
- Ask for a bug fix. The agent must ask whether to spec it or treat it as a
  no-spec change instead of silently choosing either path.

## Writes

Generated adapters, command wrappers, agent, skill and standard files only. No
project content, spec or code.
