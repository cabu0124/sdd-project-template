...
- `AGENTS.md` — the `Rule 1 — Spec First` block between `<!-- sdd:rule1:start -->`
  and `<!-- sdd:rule1:end -->` is copied verbatim into every adapter.
...
2. **Command files** — for each selected tool that has a command system, generate
   ...
   `adopt`, `sync`, `specify`, `clarify`, `plan`, `tasks`, `analyze`,
   `implement`). Each
...
- Ask for something it must refuse without a spec — *"add a `/health` endpoint"*.
  It passes if it writes the spec and stops, fails if it writes code.
- Ask for a bug fix — *"the login form submits twice on a double click"*. It
  passes if it asks whether to spec the change first, fails if it silently writes
  a spec or silently patches the code.
