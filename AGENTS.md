...
1. Find the spec in `specs/` — WHAT the product does, independent of stack and repo. Not here yet: `/sdd-sync <id>` registers it from the Spec Repository named in `.sdd/config.yml`, or `/sdd-specify` writes it when none is configured. A spec owned upstream is **read from there, never copied here**: `spec.link.yml` says which one. A local spec stops for approval; an upstream one carries its owner's approval in `status:` and is not approved again here.
